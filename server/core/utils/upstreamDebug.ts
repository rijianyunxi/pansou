...
function redactUrl(raw: string): string {
  try {
    ...
    for (const key of [...url.searchParams.keys()]) {
      if (/(token|secret|password|passwd|authorization|api[_-]?key|key)/i.test(key)) {
...
export function logUpstreamRequest(
  ...
  // Errors are always printed because a failed upstream is otherwise silently
  ...
  const prefix = `[PanHub][upstream][${phase}]`;
  const source = details.source ? ` source=${details.source}` : "";
  const attempt = details.attempt ? ` attempt=${details.attempt}` : "";
  const base = `${prefix}${source}${attempt} ${details.method} ${redactUrl(details.url)}`;
  if (phase === "error") {
