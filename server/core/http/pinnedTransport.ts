...
import type { DnsLookupRecord } from "../security/dnsResolver";
import type { IncomingMessage } from "node:http";
import type { Readable as NodeReadable } from "node:stream";
import { abortReason } from "../utils/abort";
import { normalizeIpAddress } from "../security/ipAddress";
import { byteLength } from "./responseBody";
 ...
 *
 * Cloudflare Workers 能力差异：
 * - Workers（即使开启 nodejs_compat）无法控制 DNS 解析与底层 socket 连接，
 *   `loadPinnedHttpTransport()` 返回 null，safeHttpExecutor 会对所有出站
 *   请求 fail-closed，不再切换到原生 fetch。
...
function toWebResponseHeaders(raw: IncomingMessage["headers"]): Headers {
...
function pinnedRequestOptions(
  ...
  if (init.body != null && !hasHeader("content-length")) {
    ...
    headers["content-length"] = String(byteLength(init.body));
  }
...
function toWebResponse(
  ...
  const body = hasNoBody ? null : modules.stream.Readable.toWeb(res);
  const response = new Response(body, {
    status,
    statusText: res.statusMessage ?? "",
    headers: toWebResponseHeaders(res.headers),
  });
  if (hasNoBody) res.resume();
  return response;
}
  ...
  return new Promise<Response>((resolve, reject) => {
    ...
    try {
      req = mod.request(requestInit, (res) => {
        ...
        try {
          currentResponse = res;
          const response = toWebResponse(modules, res, init.method);
          settled = true;
          resolve(response);
        } catch (error) {
          res.destroy();
...
let loadingNodeTransport: Promise<PinnedHttpTransport | null> | undefined;

async function loadNodePinnedHttpTransport(): Promise<PinnedHttpTransport | null> {
  try {
    const globalScope = globalThis as {
      process?: { versions?: { node?: unknown } };
      navigator?: { userAgent?: string };
    };
    // Cloudflare Workers 的固定标识：无自定义 DNS/socket 能力，返回不可用。
    if (globalScope.navigator?.userAgent === "Cloudflare-Workers") return null;
    // 真实 Node 运行时探测（Workers 的 nodejs_compat 可能模拟 process）。
    if (typeof globalScope.process?.versions?.node !== "string") return null;
    const [http, https, stream] = await Promise.all([
      import("node:http"),
      import("node:https"),
      import("node:stream"),
    ]);
    return createNodePinnedHttpTransport({
      http,
      https,
      stream: stream as unknown as NodePinnedTransportModules["stream"],
    });
  } catch {
    // 无 node:http/https/stream 的运行时：钉住传输不可用，调用方对域名请求 fail-closed。
    return null;
  }
}
...
export async function loadPinnedHttpTransport(): Promise<PinnedHttpTransport | null> {
  ...
  // 共享同一个初始化 Promise，避免首次并发请求把“正在加载”误判成 null。
  loadingNodeTransport ??= loadNodePinnedHttpTransport();
  try {
    cachedNodeTransport = await loadingNodeTransport;
    return cachedNodeTransport;
  } finally {
    loadingNodeTransport = undefined;
  }
}
