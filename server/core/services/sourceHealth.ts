...
export interface SourceSuccessOptions {
  resultCount?: number;
  /**
...
export type SourceFailurePhase = "network" | "http" | "business" | "parsing" | "unknown";

export interface SourceFailureOptions {
...
export type SourceDimensionKey = (typeof DIMENSION_KEYS)[number];
 ...
 */
export type SourceDimensionState = "pass" | "fail" | "empty" | "unknown";

/** 单维度健康状态。recent 环形样本同时作为持久化载体（有界字符串）。 */
export interface SourceDimensionStatus {
  state: SourceDimensionState;
...
/** 按小时聚合的检查结果桶（有界，旧 → 新）。 */
export interface SourceHealthHourlyBucket {
...
const DEFAULT_PHASE_BY_CATEGORY: Record<string, SourceFailurePhase> = {
...
export function phaseForCategory(category: string): SourceFailurePhase {
  return DEFAULT_PHASE_BY_CATEGORY[category] || "unknown";
}

/** Runtime health, latency and circuit-breaker state for one source. */
export interface SourceHealthStatus {
  ...
  dimensions?: Record<SourceDimensionKey, SourceDimensionStatus>;
  /** 按小时聚合的有界历史趋势，随快照持久化。 */
...
type InternalSourceHealthStatus = SourceHealthStatus & {
...
function recountDimension(dim: SourceDimensionStatus): void {
  let pass = 0;
  let fail = 0;
  let empty = 0;
...
function createDimension(): SourceDimensionStatus {
  return {
    state: "unknown",
    passRate: 0,
    passCount: 0,
...
function createDimensions(): Record<SourceDimensionKey, SourceDimensionStatus> {
  return {
...
function isDimensionState(value: unknown): value is SourceDimensionState {
  return (
...
function sanitizeDimension(raw: unknown): SourceDimensionStatus {
  const dim = createDimension();
  if (!raw || typeof raw !== "object") return dim;
  const source = raw as Partial<SourceDimensionStatus>;
...
function sanitizeDimensions(
  raw: unknown
): Record<SourceDimensionKey, SourceDimensionStatus> {
...
export function sanitizeHistoryBuckets(
  ...
  const byHour = new Map<number, SourceHealthHourlyBucket>();
  for (const item of raw) {
    if (!item || typeof item !== "object") continue;
    const bucket = item as Partial<SourceHealthHourlyBucket>;
...
function sanitizeHistory(raw: unknown): SourceHealthHistory {
  return {
    windowHours: MAX_HISTORY_BUCKETS,
    buckets: sanitizeHistoryBuckets(
      raw && typeof raw === "object" ? (raw as SourceHealthHistory).buckets : raw
    ),
  };
}
 ...
 */
export function sanitizeSourceHealthSnapshot(
  snapshot: Record<string, SourceHealthStatus>
): Record<string, SourceHealthStatus> {
  const out: Record<string, SourceHealthStatus> = {};
...
function cloneDimensions(
  ...
  ) as Record<SourceDimensionKey, SourceDimensionStatus>;
}

function cloneHistory(history: SourceHealthHistory): SourceHealthHistory {
  return {
...
export class SourceHealthChecker {
  private healthMap = new Map<string, InternalSourceHealthStatus>();
  ...
  private recordResponseTime(
    current: InternalSourceHealthStatus,
    responseTimeMs: number
  ): void {
  ...
  private updateParsingSuccessRate(current: InternalSourceHealthStatus): void {
  ...
  private markDimensions(
    current: InternalSourceHealthStatus,
    updates: Partial<Record<SourceDimensionKey, SourceDimensionState>>,
    now: number,
    message?: string
  ): void {
    for (const key of DIMENSION_KEYS) {
  ...
  private appendRecent(current: InternalSourceHealthStatus, success: boolean): void {
  ...
  private appendHistory(
    ...
    let bucket: SourceHealthHourlyBucket | undefined;
  ...
  /**
   * Reserves permission to execute a source. Closed circuits allow normal
   * traffic; after cooldown an open circuit allows one half-open probe only.
   */
  canExecute(sourceName: string): boolean {
    const status = this.healthMap.get(sourceName);
    ...
    if (status.circuitState === "half-open") {
      ...
      this.healthMap.set(sourceName, status);
      return true;
    }
  ...
  releaseProbe(sourceName: string): void {
    const status = this.healthMap.get(sourceName);
    if (status?.circuitState === "half-open") {
  ...
  isHealthy(sourceName: string): boolean {
    return this.healthMap.get(sourceName)?.circuitState !== "open";
  }

  getStatus(sourceName: string): SourceHealthStatus | undefined {
    const status = this.healthMap.get(sourceName);
  ...
  getAllStatus(): SourceHealthStatus[] {
    return [...this.healthMap.keys()]
      .map((name) => this.getStatus(name))
      .filter((status): status is SourceHealthStatus => !!status);
  }

  reset(sourceName: string): void {
    this.healthMap.delete(sourceName);
  }
  ...
  exportSnapshot(): Record<string, SourceHealthStatus> {
    const out: Record<string, SourceHealthStatus> = {};
   ...
   */
  importSnapshot(snapshot: Record<string, SourceHealthStatus>): void {
    const sanitized = sanitizeSourceHealthSnapshot(snapshot || {});
    for (const [name, status] of Object.entries(sanitized)) {
      const restored: InternalSourceHealthStatus = {
...
export function createSourceHealthChecker(): SourceHealthChecker {
  return new SourceHealthChecker({
    maxFailures: 5,
