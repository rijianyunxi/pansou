...
export type TgFailureKind = "channel_not_found" | "channel_private" | "network_error" | "structure_changed";
...
function classifyBody(status: number, body: string): TgChannelValidationResult["kind"] {
...
/** Validate a public channel once before storing it in the browser's user settings. */
export async function validateTgChannel(channel: string, options: { signal?: AbortSignal } = {}): Promise<TgChannelValidationResult> {
