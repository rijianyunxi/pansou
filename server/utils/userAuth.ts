...
type SessionRow = {
  ...
  created_at: number; expires_at: number; last_seen_at: number;
};
type UserRow = {
  id: number; username: string; username_normalized: string; password_hash: string; nickname: string | null;
  status: "active" | "disabled"; must_change_password: number; custom_channels_json: string;
  custom_channels_updated_at: number; last_login_at: number | null; created_at: number; updated_at: number;
};
...
export function validatePassword(password: unknown): string {
  if (typeof password !== "string" || password.length < 15 || password.length > 128) throw createError({ statusCode: 400, statusMessage: "密码长度需为 15-128 个字符" });
  return password;
}
...
function sessionExpiry(kind: "anonymous" | "user"): number {
  ...
  return now() + (kind === "user" ? policy.loginSessionDays : policy.anonymousSessionDays) * 86400000;
}
...
export function createAnonymousSession(event: H3Event): UserSessionContext {
  ...
  const result = db.run("INSERT INTO sessions(token_hash,user_id,kind,created_at,expires_at,last_seen_at) VALUES(?,?,?,?,?,?)", tokenHash(token), null, "anonymous", timestamp, sessionExpiry("anonymous"), timestamp);
  const session = db.getRow<SessionRow>("SELECT * FROM sessions WHERE id = ?", result.lastInsertRowid as number)!;
  setSessionCookie(event, token, getUserPolicy().anonymousSessionDays * 86400);
...
export function getUserSession(event: H3Event, options: { createAnonymous?: boolean; allowMustChange?: boolean } = {}): UserSessionContext {
  ...
  if (session.kind === "user" && (!user || user.status !== "active")) throw createError({ statusCode: 401, statusMessage: "登录会话已失效，请重新登录" });
...
export function rotateSession(context: UserSessionContext, event: H3Event, user: UserRow | null): UserSessionContext {
  ...
  const expiresAt = sessionExpiry(kind); const db = getSqliteDatabase();
  db.run("UPDATE sessions SET token_hash = ?, user_id = ?, kind = ?, expires_at = ?, last_seen_at = ? WHERE id = ?", tokenHash(token), user?.id ?? null, kind, expiresAt, now(), context.session.id);
  setSessionCookie(event, token, (expiresAt - now()) / 1000);
...
export function getStoredChannels(user: { custom_channels_json: string }): string[] {
  try { const value = JSON.parse(user.custom_channels_json); return Array.isArray(value) ? value.filter((x): x is string => typeof x === "string") : []; } catch { return []; }
}
...
export function publicUser(user: UserRow) {
  return { id: user.id, username: user.username, nickname: user.nickname, status: user.status, mustChangePassword: !!user.must_change_password, channels: getStoredChannels(user), lastLoginAt: user.last_login_at, createdAt: user.created_at };
}
