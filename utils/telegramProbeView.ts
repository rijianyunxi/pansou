...
/**
 * 网盘 / 磁力链接特征，复用服务端 parseChannelPage 的主机名口径
 * （pan.baidu.com、aliyundrive/alipan、quark、uc、123pan、xunlei、189、115、139）。
 * 主机名必须紧跟在 `//`（scheme 后）或某个子域标签之后，且后面不能继续接
 * 域名字符，避免把 "x115.com"、"115.com.evil.com" 这类无关域名误判。
 */
const NETDISK_HOSTS =
  "(?:pan\\.baidu\\.com|aliyundrive\\.com|alipan\\.com|pan\\.quark\\.cn|drive\\.uc\\.cn|123pan\\.com|pan\\.xunlei\\.com|cloud\\.189\\.cn|115\\.com|yun\\.139\\.com)";
const URL_TAIL = "[A-Za-z0-9._~:/?#[\\]@!$&'()*+,;=%-]*";
const LINK_SOURCE =
  `magnet:\\?xt=urn:btih:[A-Za-z0-9]{8,}(?:&[A-Za-z0-9.%_-]+=[^&\\s"'<>]*)*|ed2k://\\|file\\||https?://(?:[A-Za-z0-9-]+\\.)*${NETDISK_HOSTS}(?![A-Za-z0-9.-])${URL_TAIL}`;
...
/** 兜底归一化 stages：字段缺失 / 旧版本数据时给默认值，避免渲染报错。 */
export function normalizeTgStages(stages: unknown): TgStageRow[] {
  if (!Array.isArray(stages)) return [];
  return stages.map((raw) => {
    const item = (raw ?? {}) as Record<string, unknown>;
    const stage = item.stage === "jina" ? "jina" : "direct";
    const duration = Number(item.durationMs);
    const bytes = Number(item.bytes);
    return {
      stage,
      label: stage === "jina" ? "Jina 镜像" : "Telegram 直连",
...
/** 相对最长阶段的耗时条宽度（%，限制在 2–100 之间；缺省为 0）。 */
export function tgStageBarPercent(rows: TgStageRow[], durationMs: number | null): number {
  if (durationMs == null) return 0;
  const max = Math.max(
    1,
    ...rows.map((row) => row.durationMs ?? 0),
  );
  return Math.min(100, Math.max(2, Math.round((durationMs / max) * 100)));
}

/** 字符数展示（契约里的 bytes 是响应体长度，口径为字符数）。 */
export function formatTgCharCount(value?: number | null): string {
