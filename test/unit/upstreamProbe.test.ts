...
import { probeConfiguredUpstream } from "../../server/core/services/upstreamProbe";
...
describe("safe upstream probing", () => {
  it("does not mistake HTTP 200 with business 414 for healthy", async () => {
    ...
    const result = await probeConfiguredUpstream("hunhepan", "三体");
  ...
  it("returns real normalized results and uses no redirect following", async () => {
    ...
    const result = await probeConfiguredUpstream("hunhepan", "test");
  ...
  it("keeps empty valid result distinct from a failure", async () => {
    ...
    expect((await probeConfiguredUpstream("hunhepan", "empty")).state).toBe(
      "available",
    );
  });
  it("rejects redirects without requesting their destination", async () => {
    ...
    expect((await probeConfiguredUpstream("nyaa", "test")).state).toBe("error");
  ...
  it("marks HTML transport-only adapters as unconfirmed", async () => {
    ...
    expect((await probeConfiguredUpstream("duoduo", "test")).state).toBe(
      "warning",
    );
  });
  it("parses Nyaa results rather than accepting any HTML page", async () => {
    ...
    expect((await probeConfiguredUpstream("nyaa", "test")).results[0]!.title).toBe(
      "Test",
    );
  });
  it("fetches PanSearch data only at the fixed host", async () => {
    ...
    vi.stubGlobal("fetch", mock);
    const result = await probeConfiguredUpstream("pansearch", "a&b");
    expect(result.state).toBe("available");
  ...
  it("limits response bytes and rejects malformed JSON", async () => {
    ...
    expect((await probeConfiguredUpstream("hunhepan", "test")).message).toContain(
      "1 MiB",
    );
    expect((await probeConfiguredUpstream("hunhepan", "test")).message).toContain(
      "不是预期的 JSON",
  ...
  it("rejects unknown targets before network access", async () => {
    const mock = vi.fn();
    vi.stubGlobal("fetch", mock);
    await expect(probeConfiguredUpstream("custom-123", "test")).rejects.toThrow(
      "Unknown upstream",
    );
