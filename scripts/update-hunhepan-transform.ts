...
const row = db.prepare("SELECT * FROM resource_sources WHERE id='hunhepan'").get() as any;
if (row) {
  ...
  db.prepare('UPDATE resource_sources SET request_json=?,transform=?,updated_at=? WHERE id=? AND transform=? AND request_json=?').run(JSON.stringify(request), transform, Date.now(), row.id, row.transform, row.request_json);
}
db.close();
