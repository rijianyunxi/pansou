<template>
  <div class="scope-control" role="group" aria-label="搜索范围">
    <button type="button" :aria-pressed="!modelValue" :disabled="disabled" @click="$emit('update:modelValue', false)">
    ...
    <button type="button" :aria-pressed="modelValue" :disabled="disabled" @click="$emit('update:modelValue', true)">
...
defineProps<{ modelValue: boolean; count: number; disabled?: boolean }>();
...
button:hover:not(:disabled) { color: var(--primary); }
