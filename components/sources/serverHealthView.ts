 ...
 * 响应载荷类型由监控接口直接提供。
 *
 * 当前无导入方：2026-09-18 孤儿接口扫描确认，仓库内没有任何文件 import 本模块。
 * 它是 components/upstreams/ -> components/sources/ 改名时被搬过来、
 * 但消费方（健康面板 UI）已删除的遗留。
 * 它服务的 /api/source-health 已于 2026-09-18 按指示删除（数据与 admin 门禁的
 * /api/monitor 完全重复），因此本模块更无存在理由。
 * 保留待定；若确认无用可直接删除本文件。
 */
